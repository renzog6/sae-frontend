...
export function UnitDialog({ accessToken, open, onOpenChange, mode, unit }: UnitDialogProps) {
  ...
  const onSubmit = (data: UnitFormData) => {
    if (mode === "create") {
      ...
      updateUnit(
        { id: unit.id, data: { name: data.name, abbreviation: data.abbreviation } },
        {
          onSuccess: () => {
  ...
  return (
    <FormDialog
      ...
      description={mode === "create" ? "Completa los datos para crear una nueva unidad." : "Modifica los datos de la unidad seleccionada."}
    >
      <UnitForm
        onSubmit={onSubmit}
        isLoading={creating || updating}
        defaultValues={mode === "edit" && unit ? { name: unit.name, abbreviation: unit.abbreviation } : undefined}
        isEdit={mode === "edit"}

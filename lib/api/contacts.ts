...
export class ContactsService {
  static async getContacts(accessToken: string, params?: { page?: number; limit?: number }) {
    const query = new URLSearchParams();
    if (params?.page) query.set("page", String(params.page));
    if (params?.limit) query.set("limit", String(params.limit));
    const qs = query.toString();
    const response = await ApiClient.request<PaginatedResponse<Contact> | Contact[]>(
      `/contacts${qs ? `?${qs}` : ""}`,
  ...
  static async getContactsByCompany(accessToken: string, companyId: number, params?: { page?: number; limit?: number }) {
    const query = new URLSearchParams();
    if (params?.page) query.set("page", String(params.page));
    if (params?.limit) query.set("limit", String(params.limit));
    const qs = query.toString();
  ...
  static async getContactsByPerson(accessToken: string, personId: number, params?: { page?: number; limit?: number }) {
    const query = new URLSearchParams();
    if (params?.page) query.set("page", String(params.page));
    if (params?.limit) query.set("limit", String(params.limit));
    const qs = query.toString();
  ...
  static async updateContact(id: number, data: UpdateContactFormData, accessToken: string) {
    const response = await ApiClient.request<Contact | ApiResponse<Contact>>(
      `/contacts/${id}`,
      {
        method: "PATCH",
